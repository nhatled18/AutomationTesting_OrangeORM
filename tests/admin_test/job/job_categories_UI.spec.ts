...
test.describe("UI Test - Job Categories Page", () => {
    ...
    test("Kiểm tra tiêu đề và nút Add", async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: 'Job Categories', exact: true })).toBeVisible();
        
        // Kiểm tra nút Add
        await expect(page.getByRole('button', { name: ' Add' })).toBeVisible();
    });

    test("Kiểm tra cấu trúc bảng Job Categories", async ({ page }) => {
        const header = page.locator('.oxd-table-header');
        
        // Kiểm tra các cột tiêu đề
        await expect(header.getByText('Job Category', { exact: true })).toBeVisible();
