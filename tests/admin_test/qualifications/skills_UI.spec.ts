...
test.describe("UI Test - Qualifications Skills", () => {
    ...
    test("Kiểm tra cấu trúc bảng danh sách kỹ năng", async ({ page }) => {
        const header = page.locator('.oxd-table-header');
        
        await expect(header.getByText('Name', { exact: true })).toBeVisible();
        await expect(header.getByText('Description', { exact: true })).toBeVisible();
